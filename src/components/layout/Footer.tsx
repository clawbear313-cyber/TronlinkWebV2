...
export default function Footer() {
  return (
    <footer style={{ backgroundColor: '#0d1219', borderTop: '1px solid #1f2d38' }}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
              {NAV_ITEMS.map((item) => (
                <li key={item.path}>
                  <Link
                    to={item.path}
                    style={{ color: '#7e8c8c', fontSize: '0.875rem', transition: 'color 200ms ease' }}
                    onMouseEnter={(e) => (e.currentTarget.style.color = '#e2dbd4')}
                    onMouseLeave={(e) => (e.currentTarget.style.color = '#7e8c8c')}
                  >
                    {item.label}
                  </Link>
                </li>
