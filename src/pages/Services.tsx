      ...
      <section className="section-pad" style={{ backgroundColor: '#0f1117' }}>
        <div className="container-site">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '3rem' }}>
                    ...
                    <div>
                      ...
                      {svc.description.map((d, di) => (
                        <p key={di} style={{ color: '#7e8c8c', lineHeight: 1.75, fontSize: '0.9375rem', marginBottom: di < svc.description.length - 1 ? '1rem' : 0 }}>{d}</p>
                      ))}
                      <p style={{ marginTop: '1.5rem', color: '#7e8c8c', fontSize: '0.8125rem', lineHeight: 1.65 }}>
                        <span style={{ color: '#c9a84c', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6875rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Best fit: </span>
                        {svc.bestFit}
                      </p>
                    </div>
                        ...
                        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                          {svc.includes.map((item) => (
                            <li key={item} style={{ display: 'flex', gap: '0.625rem', alignItems: 'flex-start' }}>
                              <span style={{ color: '#2e8fae', marginTop: '0.25rem', flexShrink: 0 }}>—</span>
                              <span style={{ color: '#7e8c8c', fontSize: '0.875rem', lineHeight: 1.55 }}>{item}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
            ...
            <div style={{ borderTop: '1px solid #1f2d38' }} />
          </div>
        </div>
      </section>

      {/* CTA */}
      <section style={{ backgroundColor: '#0d1219', borderTop: '1px solid #1f2d38' }}>
