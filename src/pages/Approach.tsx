      ...
      <section className="section-pad" style={{ backgroundColor: '#f5f4f0' }}>
        <div className="container-site">
          <Reveal style={{ marginBottom: '3.5rem' }}>
            ...
            <p style={{ color: '#3d4a52', fontSize: '1rem' }}>The foundation of how we build.</p>
          </Reveal>
          <div
            ref={principlesRef}
            style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1px', backgroundColor: '#d2cdc8' }}
          >
            {PRINCIPLES.map((p) => (
              <div key={p.num} style={{ backgroundColor: '#f5f4f0', padding: '2.5rem 2rem' }}>
                <p className="section-num" style={{ marginBottom: '1.5rem' }}>{p.num}</p>
                <h2 style={{ fontSize: '1.25rem', fontWeight: 600, marginBottom: '1rem', color: '#1a6e8a' }}>{p.title}</h2>
                <p style={{ color: '#3d4a52', lineHeight: 1.75, fontSize: '0.9375rem' }}>{p.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container-site">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '4rem', alignItems: 'start' }}>
            <Reveal>
              ...
              <p style={{ color: '#3d4a52', lineHeight: 1.75, fontSize: '0.9375rem', marginBottom: '1.25rem' }}>
