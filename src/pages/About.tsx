    ...
    <main>
      ...
      >
        ...
        <div className="container-site hero-content w-full">
          <Reveal>
            ...
            <p style={{ color: '#3d4a52', fontSize: '1.0625rem', maxWidth: '480px', lineHeight: 1.75 }}>
      ...
      <section className="section-pad" style={{ backgroundColor: '#f5f4f0' }}>
        <div className="container-site">
          ...
          >
            {VALUES.map((v) => (
              <div key={v.num} style={{ backgroundColor: '#f5f4f0', padding: '2.25rem 1.75rem' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.625rem', marginBottom: '1.25rem' }}>
                  <span style={{ color: '#1a6e8a', fontSize: '1.5rem', fontWeight: 700, lineHeight: 1 }}>{v.titleZh}</span>
                  <span style={{ color: '#3d4a52', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem' }}>{v.titleEn}</span>
                </div>
                <p style={{ color: '#3d4a52', fontSize: '0.875rem', lineHeight: 1.7 }}>{v.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Team */}
      <section className="section-pad" style={{ backgroundColor: '#eeecea', borderTop: '1px solid #d2cdc8' }}>
        <div className="container-site">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '4rem', alignItems: 'start' }}>
            <Reveal>
              ...
              <p style={{ color: '#3d4a52', lineHeight: 1.75, fontSize: '0.9375rem' }}>
                一支由資深前端工程師與系統架構師組成的精實團隊，在長期合作中成為客戶最信賴的技術夥伴。
              </p>
            </Reveal>
            <Reveal delay={0.15}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', backgroundColor: '#d2cdc8' }}>
                {ROLES.map((role, i) => (
                  <div
                    key={role}
                    style={{
                  ...
                  >
                    ...
                    <span style={{ color: '#111318', fontWeight: 500 }}>{role}</span>
                  </div>
                ))}
              </div>
            </Reveal>
      ...
      {/* CTA */}
      <section style={{ backgroundColor: '#f5f4f0', borderTop: '1px solid #d2cdc8' }}>
