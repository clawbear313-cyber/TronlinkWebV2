...
export default function Home() {
  ...
  return (
    <main>
      ...
      >
        ...
        <div className="container-site hero-content w-full">
          <div ref={heroContentRef} style={{ maxWidth: '640px' }}>
            ...
            <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 4.5rem)', fontWeight: 700, marginBottom: '1.5rem', color: '#e2dbd4' }}>
              將概念化為形體
            </h1>
            <p style={{ color: '#7e8c8c', fontSize: '1.0625rem', lineHeight: 1.75, maxWidth: '480px', marginBottom: '2.5rem' }}>
        ...
        <div className="container-site">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '4rem', alignItems: 'start' }}>
            <Reveal>
              ...
              <p style={{ color: '#7e8c8c', lineHeight: 1.75, fontSize: '1rem' }}>
            ...
            <div ref={cardsRef} style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>
              {[
                ...
                >
                  ...
                  <div>
                    <p style={{ fontWeight: 600, marginBottom: '0.375rem', color: '#e2dbd4' }}>{card.title}</p>
                    <p style={{ color: '#7e8c8c', fontSize: '0.875rem', lineHeight: 1.65 }}>{card.desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* ── 02 Services ── */}
      <section className="section-pad" style={{ backgroundColor: '#0d1219' }}>
        <div className="container-site">
          ...
          <div ref={servicesRef} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1px', backgroundColor: '#1f2d38' }}>
            {SERVICES.map((svc) => (
              <div
                key={svc.id}
                style={{ backgroundColor: '#0d1219', padding: '2.5rem 2rem' }}
              >
                <p className="section-num" style={{ marginBottom: '1.25rem' }}>{svc.id}</p>
                <h3 style={{ fontSize: '1.125rem', fontWeight: 600, marginBottom: '0.5rem', color: '#e2dbd4' }}>{svc.nameZh}</h3>
                <p style={{ color: '#3d4a52', fontSize: '0.8125rem', fontFamily: 'JetBrains Mono, monospace', marginBottom: '1rem' }}>{svc.nameEn}</p>
                <p style={{ color: '#7e8c8c', fontSize: '0.875rem', lineHeight: 1.7 }}>{svc.description[0]}</p>
                <Link
                  to="/services"
                  style={{ display: 'inline-flex', alignItems: 'center', gap: '0.375rem', marginTop: '1.5rem', color: '#2e8fae', fontSize: '0.8125rem', fontWeight: 500 }}
                >
            ...
            {WORK_CASES.map((wc) => (
              ...
              >
                ...
                <div style={{ padding: '1.75rem' }}>
                  ...
                  <h3 style={{ fontSize: '1.125rem', fontWeight: 600, marginBottom: '0.25rem', color: '#e2dbd4' }}>{wc.client}</h3>
                  <p style={{ color: '#3d4a52', fontSize: '0.8125rem', fontFamily: 'JetBrains Mono, monospace', marginBottom: '0.875rem' }}>{wc.industry}</p>
                  <p style={{ color: '#7e8c8c', fontSize: '0.875rem', lineHeight: 1.65 }}>{wc.outcome}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      {/* ── CTA Banner ── */}
      <section style={{ backgroundColor: '#0d1219', borderTop: '1px solid #1f2d38', borderBottom: '1px solid #1f2d38' }}>
        <div className="container-site" style={{ paddingTop: '5rem', paddingBottom: '5rem', textAlign: 'center' }}>
          <Reveal>
            ...
            <p style={{ color: '#7e8c8c', fontSize: '1rem', marginBottom: '2.5rem' }}>讓我們一起討論您的下一個專案。</p>
